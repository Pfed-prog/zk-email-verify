...
if (process.argv.length < 3 || process.argv.length > 6) {
  console.log("usage");
  console.log(
    "compile comma,separated,list,of,circuits,or,--all [`true` if deterministic / `false` if not] [skip-r1cswasm to skip recompiling the circuit, anything else to recompile] [skip-zkey to skip recompiling the zkey, anything else to do it]"
  );
  process.exit(1);
}
...
for (let circuitName of circuitsList.split(",")) {
  ...
  if (!process.env["beacon"]) {
    console.log(
      "INSECURE ZKEY: You don't have a beacon in your .env file, test beacon assigned"
    );
    beacon = "test";
  } else {
  ...
  try {
    ...
    if (process.argv.length >= 4 && process.argv[4] === "skip-r1cswasm") {
      ...
      execSync(`circom ${circuitNamePrimary}.circom --r1cs --wasm --sym`, {
        stdio: "inherit",
      });
      execSync(
        `node --max-old-space-size=614400 ./../node_modules/snarkjs r1cs info ${circuitNamePrimary}.r1cs`,
        { stdio: "inherit" }
      );
      execSync(
        `cp ${circuitNamePrimary}_js/${circuitNamePrimary}.wasm ${circuitNamePrimary}.wasm`,
        { stdio: "inherit" }
      );
      execSync(
        `node ${circuitNamePrimary}_js/generate_witness.js ${circuitNamePrimary}.wasm inputs/input_${circuitNamePrimary}.json ${circuitNamePrimary}.wtns`,
        {
          stdio: "inherit",
        }
      );
    ...
    if (
      ...
      process.argv[5] === "skip-zkey" &&
      process.argv[4] === "skip-r1cswasm"
    ) {
      ...
      execSync(
        `node --max-old-space-size=614400 ${snarkJSPath} zkey verify ${circuitNamePrimary}.r1cs ../../powersoftau/powersOfTau28_hez_final_${ptau_size}.ptau circuit.zkey`,
        {
          stdio: "inherit",
        }
      );
      execSync(`mkdir -p ${cwd}/circuits/${circuitNamePrimary}/keys/`, {
        stdio: "inherit",
      });
      execSync(
        `node --max-old-space-size=614400 ${snarkJSPath} zkey export verificationkey circuit.zkey ${circuitNamePrimary}/keys/verification_key.json`,
        {
          stdio: "inherit",
        }
      );
      ...
      execSync(`mkdir -p ${cwd}/circuits/${circuitNamePrimary}/compiled/`, {
        stdio: "inherit",
      });
      execSync(`mkdir -p ${cwd}/circuits/${circuitNamePrimary}/keys/`, {
        stdio: "inherit",
      ...
      execSync(
        `cp ${circuitNamePrimary}.wasm ${cwd}/circuits/${circuitNamePrimary}/compiled/circuit.wasm`,
        { stdio: "inherit" }
      );
      execSync(
        `cp circuit.zkey ${cwd}/circuits/${circuitNamePrimary}/keys/circuit_final.zkey`,
        { stdio: "inherit" }
      );
    ...
    fs.copyFileSync(
      `${cwd}/circuits/contracts/verifier.sol`,
      `${cwd}/src/contracts/src/${circuitNamePrimary}Verifier.sol`
    );
    fs.copyFileSync(
      `${cwd}/circuits/${circuitNamePrimary}/compiled/circuit.wasm`,
      `${cwd}/src/circuits/${circuitNamePrimary}_circuit.wasm`
    );
    fs.copyFileSync(
      `${cwd}/circuits/${circuitNamePrimary}/keys/circuit_final.zkey`,
      `${cwd}/src/circuits/${circuitNamePrimary}_circuit_final.zkey`
    );
    fs.copyFileSync(
      `${cwd}/circuits/${circuitNamePrimary}/keys/verification_key.json`,
      `${cwd}/src/circuits/${circuitNamePrimary}_verification_key.json`
    );
  } catch (error) {
    console.log(error);
    process.exit(1);
